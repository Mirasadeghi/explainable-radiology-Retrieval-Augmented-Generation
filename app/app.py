...
# ------------------------------
ARTIFACT_DIR = os.environ.get("MEDVISION_ARTIFACT_DIR", "artifacts")

def artifact_path(fname: str) -> str:
    """Prefer artifacts/<fname> if present, otherwise fall back to <fname>."""
...
def load_resources():
    ...
    try:
        ...
        if os.path.exists("dataset_final_fixed.pkl"):
            with open(artifact_path("dataset_final_fixed.pkl"), "rb") as f:
                db_df = pickle.load(f)
        elif os.path.exists("dataset_with_embeddings_fixed.pkl"):
            with open(artifact_path("dataset_with_embeddings_fixed.pkl"), "rb") as f:
                db_df = pickle.load(f)
        elif os.path.exists("dataset_with_embeddings.pkl"):
            with open(artifact_path("dataset_with_embeddings.pkl"), "rb") as f:
                db_df = pickle.load(f)
        else:
        ...
        if os.path.exists("smart_adapter.pkl"):
            with open(artifact_path("smart_adapter.pkl"), "rb") as f:
                ai_classifier = pickle.load(f)

        # 3) MultiLabelBinarizer
        if os.path.exists("multilabel_binarizer.pkl"):
            with open(artifact_path("multilabel_binarizer.pkl"), "rb") as f:
                mlb = pickle.load(f)
        ...
        if os.path.exists("calibrators.pkl") and mlb is not None:
            try:
                with open(artifact_path("calibrators.pkl"), "rb") as f:
                    cpack = pickle.load(f)
        ...
        if os.path.exists("per_class_thresholds.pkl") and mlb is not None:
            with open(artifact_path("per_class_thresholds.pkl"), "rb") as f:
                pack = pickle.load(f)
